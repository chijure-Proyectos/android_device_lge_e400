...
int Overlay::getBppFromFormat(const Format format)
{
    ...
    case FORMAT_YUV420P:
        return 12;
    default:
        ALOGV("%s: unhandled color format %d", __FUNCTION__, format);
    }
    return 32;
}
...
Overlay::Format Overlay::getFormatFromString(const char* name)
{
    ...
    ALOGV("%s: unhandled color format %s", __FUNCTION__, name);
...
Overlay::Overlay(uint32_t width, uint32_t height, Format format, QueueBufferHook queueBufferHook, void *data) :
    ...
    mFormat(format)
{
    ALOGV("%s: Init overlay", __FUNCTION__);
    ...
    if (fd < 0) {
        ALOGV("%s: Cannot create ashmem region", __FUNCTION__);
        return;
    }
    ...
    for (uint32_t i = 0; i < NUM_BUFFERS; i++) {
        ...
        if (mBuffers[i].ptr == MAP_FAILED) {
            ALOGV("%s: Failed to mmap buffer %d", __FUNCTION__, i);
            mBuffers[i].ptr = NULL;
            continue;
        }
        mQueued[i] = false;
    ...
    ALOGV("%s: Init overlay complete", __FUNCTION__);
    ...
    if (mNumFreeBuffers < NUM_MIN_FREE_BUFFERS) {
        ...
        if (index >= 0) {
            ...
            *intBuffer = index;
            ...
            ALOGV("%s: inconsistent queue state", __FUNCTION__);
            rv = NO_MEMORY;
        }
    }
...
status_t Overlay::queueBuffer(overlay_buffer_t buffer)
{
    ...
    if (index > NUM_BUFFERS) {
        ALOGV("%s: invalid buffer index %d", __FUNCTION__, index);
    ...
    if (mNumFreeBuffers < NUM_BUFFERS) {
        ...
        ALOGV("%s: Attempt to queue more buffers than we have", __FUNCTION__);
...
void* Overlay::getBufferAddress(overlay_buffer_t buffer)
{
    ...
    //ALOGV("%s: fd=%d, length=%d. offset=%d, ptr=%p", __FUNCTION__, mBuffers[index].fd,
...
void Overlay::destroy()
{
    ...
    for (uint32_t i = 0; i < NUM_BUFFERS; i++) {
        if (mBuffers[i].ptr != NULL && munmap(mBuffers[i].ptr, mBuffers[i].length) < 0) {
            ALOGV("%s: unmap of buffer %d failed", __FUNCTION__, i);
        } else {
            mBuffers[i].ptr = NULL;
        }
        if (mBuffers[i].fd > 0) {
            if (fd > 0 && fd != mBuffers[i].fd) {
                ALOGV("%s: multiple fd detected, closing fd %d...", __FUNCTION__, fd);
                close(fd);
            }
            fd = mBuffers[i].fd;
            mBuffers[i].fd = 0;
