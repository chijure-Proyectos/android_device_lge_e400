                         ...
                         int x, int y, int w, int h)
{
    ...
    if (fb_fd < 0) {
       ALOGV("CameraHAL_CopyBuffers_Hw: Error opening /dev/graphics/fb0\n");
       return false;
    }
...
{
   ALOGV("CameraHAL_DataTSCb: timestamp:%lld now:%lld msg_type:%d user:%p\n",
   ...
   if (origDataTS_cb != NULL && origCamReqMemory != NULL) {
	 ...
	 sentFrames.push_back(clientData);
         ...
         ALOGV("CameraHAL_DataTSCb: ERROR allocating memory from client\n");
      }
   }
}
                           ...
                           struct preview_stream_ops *window)
{
   ...
   if (device == NULL) {
      ALOGV("qcamera_set_preview_window : Invalid device.\n");
      return -EINVAL;
   } else {
      ALOGV("qcamera_set_preview_window : window :%p\n", window);
      mWindow = window;
                   ...
                   hw_device_t** device)
{
   ...
   ALOGV("qcamera_device_open: name:%s device:%p cameraId:%d\n", 
